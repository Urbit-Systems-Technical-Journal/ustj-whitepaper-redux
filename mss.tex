...
\fancyfoot[L]{{\footnotesize
              ...
              {\it Urbit Systems Technical Journal} I:1 (2024):  1–45. \\
              ~ \\
...
The \lstinline[style=inlinecode]{.^} dotket rune no longer blocks until complete.  Because the agent model expects non-blocking I/O, a single dotket could make an entire agent stuck if it can't be completed.  While a lot of work has gone into fleshing out the scry namespace, core devs are now entertaining the notion that the expectation of synchronous access to the namespace might not be worth it for agents.  An Urbit Improvement Proposal has been submitted that would remove the dotket rune in favor of a move-based interface to the namespace.
